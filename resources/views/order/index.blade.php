    ...
    <script>
        $(document).ready(function() {
                        ...
                        $('#order-detail-body').html(`
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="p-3.5 rounded-xl bg-white dark:bg-slate-700/30 border border-slate-200/80 dark:border-slate-700 text-xs space-y-1">
                                    ...
                                    ${o.payment_method === 'bank_transfer' ? `<div class="font-semibold ${Number(o.pay_status) === 1 ? 'text-emerald-600' : 'text-amber-600'}">${Number(o.pay_status) === 1 ? 'Đã đối soát chuyển khoản' : 'Chờ đối soát chuyển khoản'}</div>` : ''}
                                </div>
                            </div>

                            ${o.payment_method === 'bank_transfer' && Number(o.pay_status) !== 1 && !['cancelled', 'returned'].includes(o.order_status)
                                ? `<button type="button" data-order-id="${o.id}" data-order-code="${o.order_code}" class="btn-confirm-payment rounded-lg bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700">Xác nhận đã nhận tiền</button>`
                                : ''}
            ...
            $(document).on('click', '.btn-confirm-payment', function() {
                ...
                if (!window.confirm('Bạn đã đối chiếu sao kê và xác nhận nhận đủ tiền cho đơn ' + orderCode + '?')) return;

                $button.prop('disabled', true).addClass('opacity-50').text('Đang ghi nhận...');
                $.ajax({
                    ...
                    error: function(xhr) {
                        $button.prop('disabled', false).removeClass('opacity-50').text('Xác nhận đã nhận tiền');
                        window.showAjaxError(xhr);
                    }
                });
            });
