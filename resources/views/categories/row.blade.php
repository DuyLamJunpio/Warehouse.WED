{{--
    Một dòng danh mục. Dùng chung cho danh mục gốc và danh mục con.
    $item    : Categories (đã withCount('products'))
    $isChild : true = danh mục con, thụt lề và cỡ chữ nhỏ hơn
--}}
@php $isChild = $isChild ?? false; @endphp
<tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
    <td class="p-4 whitespace-nowrap">
        @if ($item->image)
            <img class="w-10 h-10 rounded-lg object-cover" src="{{ Storage::url($item->image) }}"
                alt="{{ $item->name }}">
        @else
            <div class="flex items-center justify-center w-10 h-10 text-gray-400 bg-gray-100 rounded-lg dark:bg-gray-700">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd"
                        d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z"
                        clip-rule="evenodd"></path>
                </svg>
            </div>
        @endif
    </td>

    <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
        <div class="{{ $isChild ? 'pl-8' : '' }}">
            <div class="{{ $isChild ? 'text-sm font-medium' : 'text-base font-semibold' }} text-gray-900 dark:text-white">
                @if ($isChild)
                    <span class="mr-1 text-gray-400">└</span>
                @endif
                {{ $item->name }}
            </div>
            @if ($item->description)
                <div class="max-w-xs text-xs truncate">{{ $item->description }}</div>
            @endif
        </div>
    </td>

    <td class="p-4 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">{{ $item->slug }}</td>

    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
        {{ $item->products_count ?? 0 }}
    </td>

    <td class="p-4 whitespace-nowrap">
        <div class="flex items-center space-x-1">
            <span class="w-6 text-sm text-gray-500 dark:text-gray-400">{{ $item->sort_order }}</span>
            <button type="button" data-id-categories="{{ $item->id }}" data-direction="up"
                class="reorderCategoriesButton p-1 text-gray-500 rounded hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-600"
                title="Lên">
                <svg class="w-3 h-3" fill="none" viewBox="0 0 10 6" xmlns="http://www.w3.org/2000/svg">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 5 5 1 1 5" />
                </svg>
            </button>
            <button type="button" data-id-categories="{{ $item->id }}" data-direction="down"
                class="reorderCategoriesButton p-1 text-gray-500 rounded hover:bg-gray-200 dark:text-gray-400 dark:hover:bg-gray-600"
                title="Xuống">
                <svg class="w-3 h-3" fill="none" viewBox="0 0 10 6" xmlns="http://www.w3.org/2000/svg">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="m1 1 4 4 4-4" />
                </svg>
            </button>
        </div>
    </td>

    <td class="p-4 text-base font-medium {{ $item->status == 1 ? 'text-green-500' : 'text-red-600' }} whitespace-nowrap">
        {{ $item->status == 1 ? 'Đang sử dụng' : 'Đã huỷ' }}
    </td>

    <td class="p-4 space-x-2 whitespace-nowrap">
        <button type="button" data-drawer-target="drawer-update-product-default"
            data-drawer-show="drawer-update-product-default" aria-controls="drawer-update-product-default"
            data-drawer-placement="right" data-id-categories="{{ $item->id }}"
            data-name-categories="{{ $item->name }}" data-status-categories="{{ $item->status }}"
            data-parent-categories="{{ $item->parent_id }}" data-description-categories="{{ $item->description }}"
            class="editCategoriesButton inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
            <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"></path>
                <path fill-rule="evenodd"
                    d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"
                    clip-rule="evenodd"></path>
            </svg>
            Sửa
        </button>
        <button type="button" data-drawer-target="drawer-delete-product-default"
            data-drawer-show="drawer-delete-product-default" aria-controls="drawer-delete-product-default"
            data-drawer-placement="right" data-id-categories="{{ $item->id }}"
            data-name-categories="{{ $item->name }}"
            class="deleteCategoriesButton inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
            <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd"
                    d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z"
                    clip-rule="evenodd"></path>
            </svg>
            Xóa
        </button>
    </td>
</tr>
